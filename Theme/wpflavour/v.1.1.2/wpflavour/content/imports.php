...
//Theme_Content_Import
	...
	{	
		...
		{	
			...
			if ($key != 'item_format')
			{
				...
				if(strpos($key,'req')==true && $value == 'on') 
				{
					...
					if(strtolower($con_info['wpfl_item_id']) == "email" && strtolower($con_info['is_hidden']) == 'false')
					{
		...
		$post_mail = array(
			'subject' => sprintf(
				_x( '%1$s "%2$s"', 'mail subject', 'wpflavour' ),
				get_bloginfo( 'name' ), '['.$mail_arr[3].']' ),
			'sender' => sprintf( '['.$mail_arr[1].'] <%s>', '['.$mail_arr[2].']' ),
			'body' =>
				sprintf( __( 'From: %s', 'wpflavour' ),
					'['.$mail_arr[1].'] <['.$mail_arr[2].']>' ) . "\n"
				. sprintf( __( 'Subject: %s', 'wpflavour' ),
					'['.$mail_arr[3].']' ) . "\n\n"
				. __( 'Message Body:', 'wpflavour' )
					. "\n" . '[your-message]' . "\n\n"
				. '-- ' . "\n"
				. sprintf( __( 'This e-mail was sent from a contact form on %1$s (%2$s)',
					'wpflavour' ), get_bloginfo( 'name' ), home_url() ),
