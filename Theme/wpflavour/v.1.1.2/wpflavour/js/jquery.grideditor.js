...
$.fn.gridEditor = function( options ) {
    ...
    self.each(function(baseIndex, baseElem) {
        ...
        function setup() {
          ...
          var content = pass_data.content;
              canvas.empty().html(content).show();
            ...
            $.each(settings.new_row_layouts, function(j, layout) {
                ...
                layout.forEach(function(i) {
                    icon += '<div class="column col-xs-' + i + ' col-' + i + '"/>';
                });
                icon += '</div>';
                btn.append(icon);
            });
            ...
            // Buttons on right
            var layoutDropdown = $('<div class="dropdown pull-right float-right ge-layout-mode">' +
                ...
                .appendTo(wrapper)
            ;
            var btnGroup = $('<div class="btn-group pull-right float-right"/>')
                .appendTo(wrapper)
            ;
        ...
        function setColSize(col, colClass, size) {
            ...
            if (reResult && parseInt(reResult[2]) !== size) {
            	...
            	col.addClass(colClass + size).removeClass(reResult[1]);
...
(function() {
    jQuery.fn.gridEditor.RTEs.ckeditor = {
...
(function() {
    ...
    jQuery.fn.gridEditor.RTEs.summernote = {
...
(function() {
    jQuery.fn.gridEditor.RTEs.tinymce = {
        init: function(settings, contentAreas) {
            if (!window.tinymce) {
            ...
            if (google_font == '' || google_font == null)
            {						
				google_font = [];
			}
                ...
                if (!contentArea.hasClass('active')) {
                    ...
                    var configuration = jQuery.extend(
                        ...
                        (settings.tinymce && settings.tinymce.config ? settings.tinymce.config : {}),
                        {
                            ...
                            convert_urls : true,
                            plugins: ["wplink hr charmap paste", "wordpress", "wpeditimage"],
							...
							selector: ".ge-content.active",
							font_formats: fontformats,
							content_css: google_font,
                    ...
                    var tiny = tinymce.init(configuration);
